...
shaka.media.StreamingEngine.prototype.addRequestTracker_ =
  ...
  const ContentType = shaka.util.ManifestParserUtils.ContentType;
  if (contentType != ContentType.VIDEO) return;
...
shaka.media.StreamingEngine.prototype.consumeRequestTrackingEvents_ =
  ...
  let trackers =
    this.requestTrackers_.filter(function(tracker) {
      return tracker.totalBytes > 0 && tracker.contentType == ContentType.VIDEO;
    });

  let half = true;
  trackers.forEach(function(tracker, index) {
    if (tracker.contentType == ContentType.VIDEO && tracker.reference.endTime >= playheadTime) half = false;
  }.bind(this));
