...
shaka.media.StreamingEngine.prototype.init = function() {
  ...
  this.playerInterface_.netEngine.addEventListener('progress',
      /** @type {EventListener} */ (this.netProgressEventHandler_));
...
shaka.media.StreamingEngine.prototype.initSourceBuffer_ = function(
  ...
  let fetchInit = this.fetch_(mediaState.stream.initSegmentReference, mediaState.type);
 ...
 *   reference
 * @param {shaka.util.ManifestParserUtils.ContentType}
 *   contentType
 *
 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.addRequestTracker_ =
    function(reference, request, contentType) {
...
/**
 * Updates the appropriate request tracker (if existent)
 * based on a net progress event.
 * @param {ProgressEvent} progressEvent
 * @private
 * @suppress {missingProperties}
 */
shaka.media.StreamingEngine.prototype.updateRequestTrackers_ =
  ...
  // look for tracker
  let requestTracker = this.requestTrackers_.filter(function(tracker) {
 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.consumeRequestTrackingEvents_ =
    function() {
  if (!this.playerInterface_) return;
  let playerIsNotBuffering =
    !this.playerInterface_.playheadObserver.isBuffering();
  ...
  /*const rebufferingGoal = this.manifest_ && this.config_ ? shaka.util.StreamUtils.getRebufferingGoal(
      this.manifest_, this.config_, this.bufferingGoalScale_) : 0,
    playheadTime = this.playerInterface_.playhead.getTime(),
    ContentType = shaka.util.ManifestParserUtils.ContentType;*/

  let loadedMediaTime = 0, totalBytes = 0;

  let trackers =
    this.requestTrackers_.filter(function(tracker) {
      return tracker.totalBytes > 0;
    });

  let half = false;
  /*trackers.forEach(function(tracker, index) {
    if (tracker.contentType == ContentType.VIDEO && tracker.reference.endTime >= rebufferingGoal + playheadTime) half = false;
  }.bind(this));*/

  trackers.forEach(function(tracker, index) {
    let isNotLast = index !== (trackers.length - 1);
    let isDone = tracker.endTime !== null;

    // if we are not buffering and this is not the last tracker added,
    // and the request is done, removes this tracker from the list.
    if (playerIsNotBuffering && isNotLast && isDone) {
      let index2 = -1;
      for (let i = 0; i < this.requestTrackers_.length; ++i) {
        if (this.requestTrackers_[i] == tracker) {
         index2 = i;
         break;
        }
      }
      this.requestTrackers_.splice(index2, 1);
    }

    loadedMediaTime += tracker.loadedBytes;
    totalBytes += tracker.totalBytes;
  }.bind(this));


  let event = new shaka.util.FakeEvent('segmentbuffering', {
    'bufferingProgress': Math.round(loadedMediaTime / totalBytes * 1000) / 10 * (half ? 0.5 : 1)
  });
