...
shaka.Player = function(video, opt_dependencyInjector) {
  ...
  /** @private {number} */
  this.currentAudioTrack_ = this.config_.preferredTrack;
...
shaka.Player.prototype.defaultConfig_ = function() {
  ...
  if (navigator.connection) {
    ...
    if (NetworkInformation && NetworkInformation.saveData) {
      abrMaxHeight = 360;
    }
  }
...
shaka.Player.prototype.chooseVariant_ = function(variants) {
  ...
  if (!this.config_.abr.enabled) return variants[0];
...
shaka.Player.prototype.onChooseStreams_ = function(period) {
  ...
  if (this.currentAudioTrack_ > -1) {
    period.variants[this.currentAudioTrack_].primary = true;
  }
